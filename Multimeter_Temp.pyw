...
def update_temp():
    global ser, connected, valid_val, ktemp
    if connected==0:
...
def data_gen():
    ...
    while True:
       t+=1
       # We want to plot only when ktemp is valid
       #line_val = ktemp
       print("In data_gen")
       print(ktemp)
...
def run(data):
    ...
    if t>-1 and valid_val==1:  # Append only if ktemp is valid
        xdata.append(t)
        ydata.append(ktemp)
...
ser = serial.Serial(
    port='COM4',            # Might need to change
    baudrate=9600,
